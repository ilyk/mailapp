...
impl MessageHeader {
    pub fn new(sender: &str, subject: &str, recipients: &[String], date: &OffsetDateTime) -> Self {
        ...
        // Avatar (28px circle) - using a simple colored box for now
        ...
        // Sender info with name and email
        let sender_info = Box::new(Orientation::Horizontal, 4);
        ...
        let email_label = Label::builder()
            .label(&format!("<{}>", sender)) // Simplified email display
            .xalign(0.0)
            .build();
        email_label.add_css_class("header-email");
        
        sender_info.append(&from_label);
        sender_info.append(&email_label);
        
        names.append(&subject_label);
        names.append(&sender_info);
    ...
    fn create_action_row() -> Box {
        ...
        let flag = ToggleButton::new();
        flag.set_icon_name("starred-symbolic");
