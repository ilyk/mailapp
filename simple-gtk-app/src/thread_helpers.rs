...
impl Thread {
    pub fn new(id: String, subject: String, messages: Vec<EmailMessage>) -> Self {
...
pub fn group_into_threads(mut msgs: Vec<EmailMessage>) -> Vec<Thread> {
    ...
    let mut buckets: HashMap<String, Vec<EmailMessage>> = HashMap::new();
    for m in with_tid {
...
struct Node { 
    mid: String, 
    msg: Option<EmailMessage>, 
    parent: Option<String>, 
    children: Vec<String> 
}

/// JWZ-style threading algorithm implementation
fn jwz_threads(messages: Vec<EmailMessage>) -> HashMap<String, Vec<EmailMessage>> {
    ...
    let mut out: HashMap<String, Vec<EmailMessage>> = HashMap::new();
    for root in roots.drain(..) {
        let tid = root.clone(); // synthetic id: root Message-ID
        let mut acc: Vec<EmailMessage> = Vec::new();
    ...
    for n in nodes.values() {
        if let Some(m) = &n.msg {
            if !m.message_id.as_ref().map(|x| seen.contains(x)).unwrap_or(false) {
                let tid = m.message_id.clone().unwrap_or_else(|| format!("midless-{}", m.id));
...
fn collect_inorder(nodes: &HashMap<String, Node>, id: &String, acc: &mut Vec<EmailMessage>) {
...
pub fn is_outgoing_message(message: &EmailMessage) -> bool {
    // Check if message is in Sent mailbox
    if message.mailbox.to_lowercase() == "sent" {
        return true;
    }
    ...
    message.from.contains("demo@example.com")
}
...
pub fn is_reply_message(message: &EmailMessage) -> bool {
